	...
	onMount(() => {
		...
		for (let i = 0; i < 12; i++) {
			initialTasks.push({
				...
				progress: Math.random() * 80, // Start before arrival
		...
		function animate(currentTime: number) {
			...
			// Reset endpoint processing state after animation
...
<div bind:this={container} class="absolute inset-0 overflow-hidden pointer-events-none">
	...
	<div
		...
		style="
			...
			height: 18px;
			transform: translate(-50%, -50%);
			background: radial-gradient(circle, rgba(56, 189, 248, 0.9) 0%, rgba(56, 189, 248, 0.5) 50%, transparent 80%);
	...
	{#each endpoints as ep, i}
		<!-- Outer glow - expands when processing -->
		<div
			...
			style="
				...
				background: radial-gradient(circle,
					{ep.processing ? 'rgba(139, 92, 246, 0.4)' : 'rgba(139, 92, 246, 0.2)'} 0%,
					transparent 70%);
			"
		></div>
		<!-- Inner core - subtle reaction when processing -->
		<div
			class="absolute rounded-full transition-all duration-300"
			style="
				left: {ep.x}%;
				top: {ep.y}%;
				width: {ep.processing ? 15 * ep.size : 12 * ep.size}px;
				height: {ep.processing ? 15 * ep.size : 12 * ep.size}px;
				transform: translate(-50%, -50%);
				background: radial-gradient(circle,
					{ep.processing ? 'rgba(167, 139, 250, 0.9)' : 'rgba(139, 92, 246, 0.6)'} 0%,
					{ep.processing ? 'rgba(139, 92, 246, 0.6)' : 'rgba(139, 92, 246, 0.3)'} 60%,
					transparent 80%);
				box-shadow: 0 0 {ep.processing ? 18 * ep.size : 10 * ep.size}px
					{ep.processing ? 'rgba(167, 139, 250, 0.5)' : 'rgba(139, 92, 246, 0.3)'};
			"
		></div>
		<!-- Processing ring animation - centered with the dot -->
		{#if ep.processing}
			<div
		...
		{#each tasks as task (task.id)}
			...
			<div
				...
				style="
					...
					transform: translate(-50%, -50%);
					background: radial-gradient(circle, hsla({task.hue}, 80%, 60%, {opacity * 0.35}) 0%, transparent 70%);
				"
			></div>
			<!-- Core dot -->
			<div
				...
				style="
					...
					opacity: {opacity};
					background: radial-gradient(circle,
						hsla({task.hue}, 90%, 75%, 1) 0%,
						hsla({task.hue}, 80%, 55%, 0.9) 100%);
					box-shadow: 0 0 {task.size * 1.5}px hsla({task.hue}, 80%, 60%, {opacity * 0.7});
				"
			></div>
		{/each}
	{/if}
